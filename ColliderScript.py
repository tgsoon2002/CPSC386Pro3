...
class ColliderComponent(COMP.BaseComponent):
        ...
        if(self.Type == 1):
            if (otherColl.Type == 1):
                return self.CompareCircleToCircle(
                    trans, otherTrans, self.Info, otherColl)
            if(otherColl.Type == 0):
    ...
    def CompareCircleToCircle(self, trans1, trans2, r1, r2):
        magnitude = self.GetMagnitude([trans1.location[0] + r1[0], trans1.location[1] + r1[1]], [
                                      trans2.location[0] + r2[0], trans2.location[1] + r2[1]])
    ...
    def CompareRectToRect(self, rTrans1, rTrans2, rInfo1, rInfo2):
        ...
        difx = (math.fabs(pos1[0] - pos2[0]), 2.0)
        dify = (math.fabs(pos1[0] - pos2[0]), 2.0)
        if pos1[0] > pos2[0]:
            if pos1[1] > pos2[1]:
                if difx < rInfo1[1] and dify < rInfo1[1]:
                    return True
            else:
                if difx < rInfo1[1] and dify < rInfo2[1]:
                    return True
        return False
